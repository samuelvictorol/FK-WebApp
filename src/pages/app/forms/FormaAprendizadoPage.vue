...
api.post('/form/forma_aprendizado')
...
async function enviar(forceOpenDialog) {
  ...
  const payload = { formContent: respostasDetalhadas, formType: 'forma_aprendizado' }
  ...
  try {
    ...
    resultado.value = {
      titulo: `Forma predominante: ${
        r.visual >= r.auditivo && r.visual >= r.cinestesico
          ? 'Visual'
          : r.auditivo >= r.cinestesico
            ? 'Auditivo'
            : 'Cinestésico'
      }`,
      descricao:
        `Visual: ${r.visual.toFixed(1)}%<br>` +
        `Auditivo: ${r.auditivo.toFixed(1)}%<br>` +
        `Cinestésico: ${r.cinestesico.toFixed(1)}%`
    }
...
function baixarPdfHistorico() {
  ...
  Utils.gerarPDF(resultado.value.titulo, html, 'forma_aprendizado_historico')
}
